...
def main():
    ...
    parser.add_argument('-d', '--debug', action='store_true',
                        help='Print some verbose information for advanced debugging purpose')
    ...
    if args.debug:
        print('Verbose mode', args.debug, file=sys.stderr, flush=True)

    # sqlite3 claims to su
    
##    db = sqlite3.connect('file://' + manifest_name + '?mode=ro&immutable=1', uri=True)
##    db = sqlite3.connect(manifest_name)

    try:
#        connection = sqlite3.connect(manifest_name)
        connection = sqlite3.connect('file://' + manifest_name + '?mode=ro&immutable=1', uri=True)
        cursor = connection.cursor()
        print("Connected to database")

        query = 'SELECT fileID, domain, relativePath, flags, file FROM Files'

##        counts = cursor.execute(query).fetchall()
        cursor = cursor.execute(query)
        record = cursor.fetchone()

        while(record != None):

#            print('Fetching single cursor')
            try:
                record = cursor.fetchone()
#                print (record)

                fileID, domain, relativePath, flags, filePlist = record
                assert '/' not in domain, repr(record)
                ...
                recordinfo = dict(fileId=fileID, domain=domain, relativePath=relativePath, flags=flags)
                ...
                assert plist == {'$archiver': 'NSKeyedArchiver',
                                 ...
                                 '$version': 100000}, \
                                 'unexpected plist toplevel\n' + pprint.pformat(dict(cursor=recordinfo, plist=plist, pl_objects=pl_objects))
                ...
                assert (len(pl_objects) >= 4 and pl_objects[0] == '$null' and isinstance(pl_objects[1], dict)), \
                        'invalid pl_objects toplevel struct\n' + pprint.pformat(dict(cursor=recordinfo, pl_objects=pl_objects))
                ...
                assert not indexed, 'Leftovers in indexed, Uid missing in pl_attrib?\n' + pprint.pformat(
                    dict(cursor=recordinfo, pl_attrib=pl_attrib, indexed=indexed))

                # Match the database cursor info, prevent path escapes (security)
#                assert pl_attrib['RelativePath'] == relativePath and '..' not in dom_path and not os.path.isabs(dom_path), \
#                    'Invalid relativepath\n' + pprint.pformat(
#                        dict(cursor=recordinfo, pl_attrib=pl_attrib))
                ...
                assert not common_missing, 'common keys missing\n' + pprint.pformat(
                    dict(cursor=recordinfo, pl_attrib=pl_attrib, common_missing=common_missing, uncommon_keys=uncommon_keys))

                # Decode extended attributes
                if 'ExtendedAttributes' in pl_attrib:
                    exattr = pl_attrib['ExtendedAttributes']
                    if hasattr(exattr,'keys'):
                ...
                assert (flags in [1, 2] and uncommon_keys.issubset({'ExtendedAttributes', 'Digest'})) or (
                    flags == 4 and uncommon_keys == {'Target'} and stat.S_ISLNK(mode)), \
                        'Unexpected object type\n' + pprint.pformat(
                            dict(cursor=recordinfo, pl_attrib=pl_attrib, uncommon_keys=uncommon_keys))
...
#                if stat.S_ISREG(mode):
                ...
                if args.extract:
                    ...
                    if stat.S_ISDIR(mode) and NO_EMPTY_DIRS:
                        if args.debug:
                            print('* skipping %r -> %r' % (src_name, dest))
                        ...
                        if existing and (stat.S_ISREG(existing.st_mode) and int(existing.st_mtime) == mtime and
                                         (existing.st_mode & 0o777) == (mode & 0o777)):
                            # assume same content
                            ...
                            if args.debug:
                                print('* copying %r -> %r' % (src_name, dest))
                            try:
                            ...
                            # For other errors
                                ...
                                if args.debug:
                                    print("Error occurred while copying file.", str(why))
                            ...
                            # For other errors
                                ...
                                if args.debug:
                                    print("Error occurred while trying to fix time.", str(why))
                            ...
                            # For other errors
                                ...
                                if args.debug:
                                    print("Error occurred while trying to fix permissions.", str(why))
